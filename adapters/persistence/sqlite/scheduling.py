...
class SchedulingMixin(StorePart):
    ...
    async def claim_scan(self, claim_key, kind, group_id, worker_id, lease_seconds=300):
        def f(c):
            now=int(time.time())
            c.execute("DELETE FROM scan_claims WHERE lease_until<=?",(now,))
            cur=c.execute("INSERT OR IGNORE INTO scan_claims VALUES(?,?,?,?,?,?)",(claim_key,kind,group_id,worker_id,now,now+int(lease_seconds)))
            c.commit()
            return cur.rowcount==1
        return await self._conn.exec(f)
    async def release_scan_claim(self, claim_key, worker_id):
        def f(c):
            cur=c.execute("DELETE FROM scan_claims WHERE claim_key=? AND worker_id=?",(claim_key,worker_id))
            c.commit()
            return cur.rowcount==1
        return await self._conn.exec(f)
