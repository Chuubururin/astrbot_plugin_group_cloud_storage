...
QQ 群原生云存储（群文件 / 群相册 / 精华消息）的统一管理插件，通过 OneBot 协议对接 NapCat 等协议端。云端为真源，本地维护可重建的元数据索引。
...
| 任务 | 任务记录（排队/运行/暂停/完成/失败）；暂停/继续/中断/撤销 |
| 群组 | 群列表管理；批量改名/备注/排序；框选多选 |
| 配置 | 插件配置中心（敏感项脱敏、重载提示） |
| 调试 | 操作构建器 / API 浏览器 / 调试日志（默认关闭，仅全局管理员） |
...
| `/cssave [群号] <标题> <正文>` | 保存文本为精华消息 |
| `/csfetch [群号] <URL> [文件名]` | 拉取外部文件到群文件 |
...
设计、接口与配置详情见 [docs/](docs/)（需求简报、架构总览、接口契约、数据与编码规范等）。
