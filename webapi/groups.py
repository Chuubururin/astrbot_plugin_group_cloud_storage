...
async def api_groups(s: Services) -> dict:
    ...
    Only groups with managed=1 are listed (groups of offline accounts are marked
    managed=0 during scanning).
    """
...
async def api_group_info(s: Services) -> dict:
    ...
    return json_response(await s.api.get_group_info(group, no_cache=no_cache))
...
async def api_group_members(s: Services) -> dict:
    ...
    if not group:
        return error_response("group required", status_code=400)
    no_cache = (await _param("no_cache", "false")).lower() in ("1", "true", "yes", "on")
    return json_response({"members": [m.__dict__ for m in await s.api.list_group_members(group, no_cache=no_cache)]})
...
async def api_group_honor(s: Services) -> dict:
    ...
    return json_response(await s.api.get_group_honor_info(group, honor_type or None))
...
async def api_group_system_msg(s: Services) -> dict:
    ...
    return json_response(await s.api.get_group_system_msg(group, only_pending, max(1, min(count, 200))))
...
async def api_groups_batch_actions(s: Services) -> dict:
    ...
    for gid in group_ids:
        if not gid or not await s.scan.is_page_managed(str(gid), managed):
            return error_response(f"group {gid} not managed", status_code=403)
    task_id = await s.queue.submit(
        "batch_groups",
        target="*",
        payload={
